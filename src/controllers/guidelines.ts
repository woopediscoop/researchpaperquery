...
import { createGuidelineSet, getGuidelineSets, deleteGuidelineSetById, deleteGuidelineById, getGuidelineById, 
    createGuideline, getGuidelineSetById, updateGuidelineSet, updateGuideline } from '../db/guidelines.js';
import {createFulfilledGuideline, getFulfilledGuidelineByGuidelineAndDirectory, updateFulfilledGuidelineByGuidelineAndDirectory} from '../db/directories.js';
...
export const getGuidelineSet = async (req: express.Request, res: express.Response) => {
    try{
...
export const deleteGuidelineSet = async (req: express.Request, res: express.Response) => {
    const glset = req.query.guidelineset;
        ...
        return res.json({message:"Deleted GuidelineSet Successfully",guidelineset:deletedGuidelineSet});
        
}

export const updateGuidelineSetById = async (req: express.Request, res: express.Response) => {
    try{
        const {id, values} = req.body;
        console.log(values)
        const updatedGuidelineSet = await updateGuideline(id, values);
        return res.json({message:"Updated successfully!",guideline:updatedGuidelineSet});
    }catch(error){
        console.log(error);
        return res.sendStatus(400);
    }
...
export const guidelinesBySetId = async (req: express.Request, res: express.Response) => {
    ...
    const GuidelinePromises = glset.Guidelines.map(async(guideline_id) => await getGuidelineById(guideline_id));
    try{
...
export const addGuidelineToSet = async (req: express.Request, res: express.Response) => {
    ...
    if(nrs.includes(nr)){
        ...
        return res.json({error:"Index "+nr+" Already exists in "+GLSet.name})
    }
    if(itemsTC.includes(itemToCheck)){
        console.log("Item "+itemToCheck+" already exists in "+GLSet.name);
        return res.json({error:"Item "+itemToCheck+" already exists in "+GLSet.name});
    }
...
export const getGuideline = async (req: express.Request, res: express.Response) => {
    try{
...
export const deleteGuideline = async (req: express.Request, res: express.Response) => {
    try{
        ...
        const GLIds = GLSet.Guidelines.filter(gl_id => gl_id != guideline.toString());
        await updateGuidelineSet(GLSet.id,{Guidelines:GLIds});

        
        const deletedGuideline = await deleteGuidelineById(guideline.toString()); 

        return res.json({message:"Deleted Guideline Successfully",guideline:deletedGuideline});
...
export const getCurrentGLSet = async (req: express.Request, res: express.Response) => {
    try{
        const sessionToken = req.cookies['USER-AUTH'];
        const user = await getUserBySessionToken(sessionToken);
        if(!user){
...
export const GetFulfilledGuideline = async(req: express.Request, res: express.Response) =>{
    try {
        const {directory, guideline} = req.body;
        const fulfilled = await getFulfilledGuidelineByGuidelineAndDirectory({guideline_id:guideline, directory_id:directory});
        if(fulfilled){
...
export const MakeOrUpdateFulfilledGuideline = async(req: express.Request, res: express.Response) =>{
    try {
        const {directory, guideline, passage, vectorquery} = req.body;
        const before = await getFulfilledGuidelineByGuidelineAndDirectory({guideline_id:guideline, directory_id:directory});
        if(before){
            if(vectorquery){
                ...
                const fulfilled = await updateFulfilledGuidelineByGuidelineAndDirectory({guideline_id:guideline, directory_id:directory, values:{
                    passages: passage,
