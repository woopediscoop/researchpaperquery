...
import { createDirectory, getDirectories, 
    getDirectoryById, deleteDirectoryById
...
export const noteCorrectPassage = 
...
export const getCorrectPassage = 
async (req: express.Request, res: express.Response) => {
    try{
...
export const makeDirectory = async (req: express.Request, res: express.Response) => {
    try{
        ...
        if(nameExists){
            return res.json({error:"The name "+name+" exists already as a directory"})
        }
        const pdfExists = directories.some(directory => directory.pdfname === pdfnameclean)
        if(pdfExists){
            const existingDir = directories.find(directory => directory.pdfname === pdfnameclean);
            return res.json({error:"The PDF "+pdfnameclean+" has already been uploaded! \nIt is under the name "+existingDir.name});
        }
        ...
        if(response.createdIndex){
            ...
            if(response.upserted){
                ...
                return res.json({error:"Uploading error! Create new Directory name please"})
            }
        }
...
export const deleteDirectory = async (req: express.Request, res: express.Response) => {
    const dir = req.query.directory;
    const deletedDir = await deleteDirectoryById(dir.toString());
    return res.json({message:"deleted directory successfully",directory:deletedDir})
}

export const getAllDirectories = async (req: express.Request, res: express.Response) => {
    try{
        const directories = await getDirectories();
...
export const getDirectory = async (req: express.Request, res: express.Response) => {
    try{
