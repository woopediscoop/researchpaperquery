...
export const createResults = (values: Record<string, any>) =>
  new ResultsModel(values).save().then((results) => results.toObject());
export const getResultsById = (id: string) => ResultsModel.findById(id);
export const getResultsByGuidelineAndDirectory = ({
  guideline_id,
  directory_id,
}) => ResultsModel.find({ Guideline: guideline_id, Directory: directory_id });
