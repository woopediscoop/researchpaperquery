...
const VectorQuerySchema = new mongoose.Schema({
 ...
 export const updateVectorQueryById = (id: string, values: Record<string,any>) => VectorQueryModel.findByIdAndUpdate(id,values);
 export const getVectorQueriesByGuideline = (guideline_id : string) => VectorQueryModel.find({Guideline:guideline_id});
 ...
 export const getVectorQueriesByUserAndGuidelineSet = (user : string) => VectorQueryModel.find({User:user});
 export const deleteVectorQueryById = (id : string) => VectorQueryModel.findOneAndDelete({_id:id});

 export const queryVectorQueries = (query : Record<string,any>) => VectorQueryModel.find(query);