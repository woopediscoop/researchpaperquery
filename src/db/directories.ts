...
const DirectorySchema = new mongoose.Schema({
 ...
 export const deleteDirectoryById = (id: String) => DirectoryModel.findOneAndDelete({_id: id});

 export const GuidelineFulfilledModel = mongoose.model('CorrectPassages', GuidelineFulfilledSchema);
 export const createFulfilledGuideline = (values: Record<string, any>) => new GuidelineFulfilledModel(values).save().then((correctPassage) => correctPassage.toObject());
 ...
 export const deleteFulfilledGuidelineById = (id : String) => GuidelineFulfilledModel.findOneAndDelete({_id:id});